...
class Banner extends Component {
	...
	transitionEnd(){
		...
		//判断左滑右滑
		let isLeftScroll = this.state.move-this.state.start<=0;
		//是否能滑动成功
		let canSwitch = Math.abs(this.state.move-this.state.start)>=X;
		//如果没有成功就return
		if(!canSwitch) return;
		...
		this.setState({
			...
			boxL: -16,
			transition: '0s'
		}, ev=>this.autoPlay());//回调当数据更新后需要掉自动轮播的函数
	}
  ...
  touchstart(ev){
	...
	this.setState({
		start:xs,
		boxL:this.refs.boxBan.offsetLeft/40,
		boxW:W,
		transition: '0s'
	})
  ...
  touchmove(ev){
	...
	//移动的距离+之前存的距离
	let l = xm - this.state.start +this.boxL;
